...
from .models import Avatar, Curso, Posteo, Profesor
...
def estudiantes(request):
    ...
    if request.method == "POST":
        ...
        if search != "":
            estudiantes = Posteo.objects.filter( Q(nombre__icontains=search) | Q(apellido__icontains=search) ).values()

            return render(request,"ProyectoCoderApp/estudiantes.html",{"estudiantes":estudiantes, "search":True, "busqueda":search})

    estudiantes = Posteo.objects.all()
...
def crear_Posteo(request):
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            posteo = Posteo(titulo=info["titulo"],cuerpo=info["cuerpo"],fecha = info["fecha"],foto = info["foto"])
            posteo.save()
...
def eliminar_estudiante(request,estudiante_id):
    ...
    posteo = Posteo.objects.get(id=estudiante_id)
    posteo.delete()
...
def editar_estudiante(request,estudiante_id):
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            posteo.nombre = info_estudiante["nombre"]
            posteo.apellido = info_estudiante["apellido"]
            posteo.email = info_estudiante["email"]
            posteo.save()

            return redirect("estudiantes")

    # get
    formulario = EstudianteFormulario(initial={"nombre":posteo.nombre, "apellido":posteo.apellido, "email": posteo.email})
